...
<body class="antialiased bg-gray-900 min-h-screen flex items-center justify-center px-4">
    ...
    <div class="relative z-10 w-full max-w-md">
        ...
        <div class="bg-gray-800 rounded-xl shadow-2xl border border-gray-700 p-8">
            <!-- Session Status -->
            @if (session('status'))
                <div class="mb-4 p-4 bg-green-900/50 border border-green-700 text-green-300 rounded-lg text-sm">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('supabase.login') }}">
                @csrf
                ...
                <div class="flex items-center justify-between mb-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="remember" class="w-4 h-4 text-indigo-600 bg-gray-700 border-gray-600 rounded focus:ring-indigo-500 focus:ring-2">
