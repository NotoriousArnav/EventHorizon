<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-200 leading-tight">
            Edit Ticket Type for "{{ $event->title }}"
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg border border-gray-700">
                <form action="{{ route('events.tickets.update', [$event, $ticketType]) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-gray-200 focus:ring-2 focus:ring-purple-600 focus:border-transparent"
                            placeholder="What's included with this ticket?">{{ old('description', $ticketType->description) }}</textarea>
                        @error('description')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="flex items-center space-x-3 cursor-pointer">
                            <input type="checkbox" name="is_free" id="is_free" value="1" 
                                {{ old('is_free', $ticketType->is_free) ? 'checked' : '' }}
                    ...
                    <!-- Price Fields -->
                    <div id="price-fields" class="{{ old('is_free', $ticketType->is_free) ? 'hidden' : '' }} space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="price" class="block text-sm font-medium text-gray-200 mb-2">Price *</label>
                                <input type="number" name="price" id="price" value="{{ old('price', $ticketType->price) }}" min="0" step="0.01" required
                                    class="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-gray-200 focus:ring-2 focus:ring-purple-600 focus:border-transparent">
                                @error('price')
                                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" name="currency" id="currency" value="{{ old('currency', $ticketType->currency) }}" maxlength="3" required
                                    class="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-gray-200 focus:ring-2 focus:ring-purple-600 focus:border-transparent"
                                    placeholder="USD">
                                @error('currency')
                                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="quantity_total" id="quantity_total" value="{{ old('quantity_total', $ticketType->quantity_total) }}" min="1"
                            class="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-gray-200 focus:ring-2 focus:ring-purple-600 focus:border-transparent"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="datetime-local" name="sales_start" id="sales_start" value="{{ old('sales_start', $ticketType->sales_start?->format('Y-m-d\TH:i')) }}"
                                class="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-gray-200 focus:ring-2 focus:ring-purple-600 focus:border-transparent">
                            @error('sales_start')
                            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="sales_end" id="sales_end" value="{{ old('sales_end', $ticketType->sales_end?->format('Y-m-d\TH:i')) }}"
                                class="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-gray-200 focus:ring-2 focus:ring-purple-600 focus:border-transparent">
                            @error('sales_end')
                            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        <label class="flex items-center space-x-3 cursor-pointer">
                            <input type="checkbox" name="requires_approval" id="requires_approval" value="1" 
                                {{ old('requires_approval', $ticketType->requires_approval) ? 'checked' : '' }}
                                class="w-5 h-5 bg-gray-900 border-gray-700 rounded text-purple-600 focus:ring-2 focus:ring-purple-600">
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $ticketType->sort_order) }}" min="0"
                            class="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-gray-200 focus:ring-2 focus:ring-purple-600 focus:border-transparent">
